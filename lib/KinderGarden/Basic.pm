...
sub _build_config {
    my $self = shift;
    my $config = YAML::Any::LoadFile( File::Spec->catfile($self->root, 'conf', 'kindergarden.yml') );
    my $local_config_file = File::Spec->catfile($self->root, 'conf', 'kindergarden_local.yml');
